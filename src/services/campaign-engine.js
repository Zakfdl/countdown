...
function isWithinSchedule(campaign, nowTz) {
  ...
  const startsOk = campaign.schedule.startAt
    ? nowTz.isAfter(dayjs.tz(campaign.schedule.startAt, campaign.timezone))
    : true;
  const endsOk = campaign.schedule.endAt
    ? nowTz.isBefore(dayjs.tz(campaign.schedule.endAt, campaign.timezone))
    : true;
...
function pageMatches(campaign, pageContext) {
  ...
  if (
    ...
    (campaign.displayRules?.productIds || []).includes(pageContext.productId)
  )
    return true;
  if (
    pageContext.collectionId &&
    (campaign.displayRules?.collectionIds || []).includes(
      pageContext.collectionId,
    )
  )
    return true;
...
function resolveTimer(campaign, nowTz) {
  if (campaign.timer.mode === "fixed" && campaign.timer.endAt) {
    ...
    if (endAt.isValid()) {
      const startedAt = dayjs.tz(
        campaign.timer.startAt ||
  ...
  if (recurring) {
    ...
    return {
      endAt: recurring,
      startedAt: recurring.subtract(durationHours, "hour"),
    };
  }
...
export function normalizeCampaign(campaign, merchant) {
  return {
    ...
    urgency: {
      ...
      text: {
        ar:
          campaign.urgency?.text?.ar ||
          campaign.urgency?.textAr ||
          t("urgency_low_stock", "ar"),
    ...
    cta: {
      ...
      label: {
        ar:
          campaign.cta?.label?.ar ||
          campaign.cta?.labelAr ||
          t("cta_shop_now", "ar"),
    ...
    expired: {
      ...
      message: {
        ar:
          campaign.expired?.message?.ar ||
          campaign.expired?.textAr ||
          t("status_offer_ended", "ar"),
...
export function resolveActiveCampaigns(merchant, pageContext = {}) {
  ...
  const nowTz = dayjs().tz(
    pageContext.timezone || merchant.timezone || "Asia/Riyadh",
  );

  const normalized = (merchant.campaigns || [])
  ...
  const activeOnly = normalized.filter((campaign) => campaign.isActive);
  const finalList = activeOnly.length > 0 ? activeOnly : normalized;
  ...
  return finalList
    .map((campaign) => {
      ...
      return {
        ...
        urgencyMessage: campaign.urgency.enabled
          ? pickLocalizedText(campaign.urgency.text, locale)
          : "",
        cta: {
          ...campaign.cta,
